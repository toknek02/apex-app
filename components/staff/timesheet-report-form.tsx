      ...
      <div style={{ backgroundColor: '#fff', border: '1px solid var(--apex-border)', borderRadius: 10, overflow: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 12, width: '100%' }}>
          ...
          <tbody>
            ...
            {!teamMode && results?.map((e, i) => (
              <tr key={e.id} style={{ backgroundColor: i % 2 ? 'var(--apex-row-alt)' : '#fff' }}>
                ...
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{[e.stage, e.task].filter(Boolean).join(' / ') || '—'}</td>
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{(e.normalMins / 60).toFixed(2)}</td>
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{(e.otMins / 60).toFixed(2)}</td>
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{e.remarks || '—'}</td>
              </tr>
            ))}
                ...
                <Fragment key={userId}>
                  {staff.entries.length === 0 ? (
                    <tr style={{ backgroundColor: '#fff' }}>
                      ...
                      <td colSpan={showActionsCol ? 9 : 8} style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px', color: 'var(--apex-muted)', fontStyle: 'italic' }}>No entries logged for this range.</td>
                    </tr>
                  ) : (
                    staff.entries.map((e, i) => (
                      <tr key={e.id} style={{ backgroundColor: i % 2 ? 'var(--apex-row-alt)' : '#fff' }}>
                        ...
                        <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{(e.normalMins / 60).toFixed(2)}</td>
                        <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{(e.otMins / 60).toFixed(2)}</td>
                        <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{formatCurrency(e.cost?.totalCost ?? 0)}</td>
                        <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>{e.remarks || '—'}</td>
                        {showActionsCol && (
                          <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }}>
                            <button
                  ...
                  <tr style={{ backgroundColor: 'var(--apex-dept-bg)' }}>
                    <td colSpan={5} style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px', textAlign: 'right', fontWeight: 700, fontSize: 11 }}>{staff.name} Subtotal</td>
                    <td style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px', fontWeight: 700, fontSize: 11 }}>{(staffNormal / 60).toFixed(2)}</td>
                    <td style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px', fontWeight: 700, fontSize: 11 }}>{(staffOt / 60).toFixed(2)}</td>
                    <td style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px', fontWeight: 700, fontSize: 11 }}>{formatCurrency(staffCost)}</td>
                    <td style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px' }} />
                    {showActionsCol && <td style={{ borderRight: '1px solid var(--apex-border)', padding: '7px 14px' }} />}
                  </tr>
                </Fragment>
              )
            })}
          ...
          {results && (teamMode ? byStaff.size > 0 : results.length > 0) && (
            <tfoot>
              <tr>
                <td colSpan={teamMode ? 5 : 4} style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px', textAlign: 'right', fontWeight: 700 }}>
                  {teamMode ? 'Project Total (all staff)' : 'Total'}
                </td>
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px', fontWeight: 700 }}>{(totalNormalMins / 60).toFixed(2)}</td>
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px', fontWeight: 700 }}>{(totalOtMins / 60).toFixed(2)}</td>
                {teamMode && <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px', fontWeight: 700 }}>{formatCurrency(totalCost)}</td>}
                <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }} />
                {showActionsCol && <td style={{ borderRight: '1px solid var(--apex-border)', padding: '9px 14px' }} />}
              </tr>
            </tfoot>
          )}
        </table>
