...
type UploadResult = {
  created: { code: string; shortName: string; title: string }[]
  skipped: { code: string; reason: string }[]
}

export function ProjectUploadModal({ trigger }: { trigger: React.ReactNode }) {
        ...
        >
          ...
          >
            <div style={{ padding: '24px 24px 0' }}>
              <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Upload Projects</h2>
              <p style={{ fontSize: 12, color: 'var(--apex-muted)', marginBottom: 16 }}>
                An .xlsx or .csv file with a header row containing "Code", "Short Name", and "Title" columns. Rows whose
                Code already exists are skipped. New projects are created as Active / Team access.
              </p>
              {error && (
                      ...
                      <div style={{ border: '1px solid var(--apex-border)', borderRadius: 6, maxHeight: 160, overflowY: 'auto' }}>
                        {result.skipped.map((s, i) => (
                          <div key={i} style={{ padding: '6px 10px', fontSize: 12, borderBottom: '1px solid var(--apex-border)' }}>
                            {s.code} <span style={{ color: 'var(--apex-muted)' }}>— {s.reason}</span>
                          </div>
                        ))}
                      </div>
                    </>
