          ...
          <tbody>
            ...
            {entries.map((e, i) => (
              <tr key={e.id} style={{ backgroundColor: i % 2 ? 'var(--apex-row-alt)' : '#fff' }}>
                ...
                <td style={{ padding: '9px 14px', fontSize: 12, maxWidth: 420, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={e.message}>
                  {e.message}
                </td>
                <td style={{ padding: '9px 14px', fontSize: 12, color: 'var(--apex-muted)', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={e.url ?? ''}>
                  {e.url ?? '—'}
                </td>
                <td style={{ padding: '9px 14px', fontSize: 12, color: 'var(--apex-muted)' }}>{e.userName ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
